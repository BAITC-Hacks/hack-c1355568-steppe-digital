...
function Dashboard({ result, onReview }: { result: AnalysisResult; onReview: (finding: Finding) => void }) {
  ...
  return <><div className="eyebrow">ОБЗОР АНАЛИЗА</div><div className="heading-row"><h1>{hasAnalysisData ? "Результаты анализа" : "Аналитические данные не предоставлены"}</h1>{result.isMock && <span className="demo-badge">DEMO / MOCK DATA</span>}</div><p className="intro">Документы ДО: {result.documents.filter(doc => doc.side === "before").length} <span className="divider">/</span> Документы ПОСЛЕ: {result.documents.filter(doc => doc.side === "after").length}</p>
    ...
    {!hasAnalysisData && <div className="warning" role="status">Сервер вернул документы, но не предоставил подразделения, функции и замечания. Нулевые счётчики не означают, что изменений и рисков нет. Проверьте ограничения обработки ниже.</div>}
    ...
    {view === "overview" && <section className="findings"><div className="section-heading"><h2>Замечания и источники</h2><div className="tabs"><button aria-pressed={filter === "all"} onClick={() => setFilter("all")}>Все</button><button aria-pressed={filter === "diagnostics"} onClick={() => setFilter("diagnostics")}>Проверка источников</button></div></div>
      {!visible.length ? <div className="empty"><h3>{hasAnalysisData ? "В этой категории нет замечаний" : "Замечания ещё не сформированы"}</h3><p>Отсутствие замечаний не подтверждает отсутствие рисков. Учитывайте полноту обработки и предупреждения сервера.</p></div> : <div className="finding-list">{visible.map(finding => <button key={finding.id} className={`finding-card ${!finding.verified ? "unverified" : ""}`} onClick={() => setSelected(finding.id)}><span className={`type-badge ${finding.type.toLowerCase()}`}>{findingLabels[finding.type]}</span><strong>{finding.title}</strong><p>{finding.explanation}</p><div className="card-bottom"><span>{finding.verified ? "Источники подтверждены" : "не подтверждено источником"}</span><span>{reviewLabels[finding.review.status]}</span><span className="source-link">Открыть источники ↗</span></div></button>)}</div>}
