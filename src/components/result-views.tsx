...
const unitLabels = { PRESERVED: "Сохранено", RENAMED: "Преобразовано: переименовано", MERGED: "Преобразовано: объединено", SPLIT: "Преобразовано: разделено", CREATED: "Создано", REMOVED: "Упразднено" };
...
export function ResultViews({ view, result, transferredOnly, onFinding }: { view: string; result: AnalysisResult; transferredOnly: boolean; onFinding: (id: string) => void }) {
  ...
  const changes = result.unitChanges.filter(change => (status === "all" || (status === "transformed" ? ["RENAMED", "MERGED", "SPLIT"].includes(change.status) : change.status === status)) && matches([...change.beforeUnitIds, ...change.afterUnitIds].map(id => units.get(id)?.name).join(" ") + " " + change.rationale));
  ...
  return <section className="result-section">
    ...
    {view !== "conclusion" && <><p className="muted">Статусы и итоги получены от сервера. Количества обозначают записи сопоставления.</p><div className="result-filters"><label>Поиск по названию и обоснованию<input type="search" value={query} onChange={event => setQuery(event.target.value)} /></label><label>Статус<select value={status} onChange={event => setStatus(event.target.value)}><option value="all">Все статусы</option>{view === "structure" && <option value="transformed">Преобразовано (переименование, объединение, разделение)</option>}{Object.entries(labels).map(([key, label]) => <option key={key} value={key}>{label} · {counts[key]}</option>)}</select></label></div></>}
