...
**Состояние реализации:** контракт `v0.2.0` опубликован в [docs/api/CONTRACT.md](../api/CONTRACT.md) и `src/shared/contract.ts`; реализованы TXT, десять этапов, clause alignment, правила структуры/функций положений БВА, lineage, checks и новые UI views. Это консервативный MVP: смысловое покрытие косвенных обязанностей, rename/merge/split и универсальное извлечение других шаблонов остаются ограничениями. Возможные потери и дублирование — диагностические кандидаты, не доказанные исчезновения. При runtime-конфигурации AI доступны embeddings/structured chat; без неё детерминированный режим явно отмечен. Подробный план ниже задаёт также критерии дальнейшего развития, а не обещание полноты текущего аудита.
